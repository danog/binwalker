...
process_client() {
	...
	if [ "$copid" = "" ]; then
		[ ! -z "${URLS[*]}" ] && {
			curl -s ${URLS[*]} -o "$NAME"
			send_message ${USER[ID]} $(binwalk "$NAME")
			rm "$NAME"
		}
		[ ! -z "${LOCATION[*]}" ] && send_location "${USER[ID]}" "${LOCATION[LATITUDE]}" "${LOCATION[LONGITUDE]}"
		case $MESSAGE in
			'/info')
				send_message "${USER[ID]}" "This is binwalker, a bot that allows you to bibwalk files."
				;;
			'/start')
				send_message "${USER[ID]}" "Binwalker bot.
Created by Daniil Gentili.
Uses binwalk (https://github.com/devttys0/bibwalk) and bashbot (https://github.com/topkecleon/telegram-bot-bash).
Contribute by sending a pull request @ https://github.com/danog/binwalker

Commands:
/start - Show this message
/info - Show a shorter info message
"
				;;
			'')
				;;
